const { ChatInputCommandInteraction, SlashCommandBuilder, EmbedBuilder, Embed, ButtonBuilder, ButtonStyle, ActionRowBuilder } = require("discord.js");
...
data: new SlashCommandBuilder()
    .setName("mediabiascheck")
    .setDescription("Check the bias ratings of various media sources.")
    .addStringOption(options => options
        .setName("url")
        .setDescription("Input the URL of the source you're trying to look up. EX: thehill.com")
        .setRequired(true)
    ),
    /**
     *
    ...
    async execute(interaction, client) {
        ...
        const urlMBFC = 'https://political-bias-database-discord-api.p.rapidapi.com/discord/MBFCdata';
        const optionsMBFC = {
        method: 'GET',
        headers: {
        ...
        const urlAS = 'https://political-bias-database-discord-api.p.rapidapi.com/discord/ASdata';
        const optionsAS = {
            method: 'GET',
            headers: {
                'X-RapidAPI-Key': `${mediabiasAPIKey}`,
        ...
        const sourceName = interaction.options.getString("url").toLowerCase();
        if (sourceName[0] === "w" || sourceName.includes("http")) {
            return interaction.reply({ephemeral: true, content: `Please use a link in the format of "thehill.com", not using "www." or "https" at the front.`})
        }
        ...
        .then(() => {
            ...
            if (ASResult) {
                ...
                descData.push(`\n🔹 All Sides Bias Rating: ` + '`' + ` ${ASResult.bias} bias ` + '`')
            } else {
            ...
            .then(() => {
                ...
                if (MBFCResult) {
                    ...
                    console.log(MBFCResult)
                    descData.push(`\n🔹 MB/FC Bias Rating: ` + '`' + ` ${MBFCResult.bias} bias ` + '`')
                    descData.push(`🔹 MB/FC Factuality Rating: ` + '`' + ` ${MBFCResult.factual} factuality ` + '`')
                    descData.push(`🔹 MB/FC Credibility Rating: ` + '`' + ` ${MBFCResult.credibility} ` + '`')
                } else {
                ...
                if (ASResult && MBFCResult) {
                    ...
                    descData.push(`No data was found on All Sides or Media Bias/Fact Check. Please double check your source's URL that you provided. If you input data correctly, and still do not receive any results, please click the "More Info" button below.`)
                }
                ...
                interaction.reply({ embeds: [mediabiasEmbed], components: [buttonRow] })
